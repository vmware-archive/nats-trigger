...
func NewNatsTriggerController(cfg NatsTriggerConfig) *NatsTriggerController {
	...
	natsInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			...
			if err == nil {
				newNatsTriggerObj := new.(*natsApi.NATSTrigger)
				oldNatsTriggerObj := old.(*natsApi.NATSTrigger)
...
func (c *NatsTriggerController) processNextItem() bool {
	...
	if err == nil {
		...
		c.queue.Forget(key)
	} else if c.queue.NumRequeues(key) < natsTriggerMaxRetries {
...
func (c *NatsTriggerController) syncNatsTrigger(key string) error {
	...
	triggerObj := obj.(*natsApi.NATSTrigger)
...
func (c *NatsTriggerController) natsTriggerObjNeedFinalizer(triggercObj *natsApi.NATSTrigger) bool {
...
func (c *NatsTriggerController) natsTriggerHasFinalizer(triggercObj *natsApi.NATSTrigger) bool {
	currentFinalizers := triggercObj.ObjectMeta.Finalizers
	for _, f := range currentFinalizers {
		if f == natsTriggerFinalizer {
			return true
...
func natsTriggerObjChanged(oldNatsTriggerObj, newNatsTriggerObj *natsApi.NATSTrigger) bool {
